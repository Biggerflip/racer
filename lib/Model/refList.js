...
function patchIdsEvent(type, segments, eventArgs, refList) {
  ...
  if (segmentsLength === idsLength) {
    ...
    if (type === 'remove') {
      // workaround: if this is a local event and the ids lookup is a
      // reference, stop this from happening twice
      if (!('$remote' in pass) && !pass.$remote) {
        var dereferenced = model._dereference(refList.idsSegments);
        if (!util.deepEqual(dereferenced, refList.idsSegments)) {
          return;
        }
      }
